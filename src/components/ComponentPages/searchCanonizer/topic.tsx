...
const TopicSearch = () => {
  ...
  const {
    ...
    selectedTopicFromAdvanceFilterAlgorithm:state?.searchSlice?.selectedTopicFromAdvanceFilterAlgorithm,
    selectedTopicFromAdvanceFilterAlgorithmRecords:state?.searchSlice?.selectedTopicFromAdvanceFilterAlgorithmRecords,
  ...
  const TopicItem = ({ item, searchValue, isAlgorithm }) => {
    const topicLink = isAlgorithm
      ? `/topic/${item?.topic_num}-${replaceSpecialCharacters(item?.type_value, "-")}/1-Agreement`
      ...
      <li className="flex flex-col py-3 first:pt-0 border-b border-canGrey2 last:border-none last:pb-0">
        <Link href={topicLink} passHref>
          <div className="flex justify-between items-center">
            <label className="text-base font-medium text-canBlack flex !mb-2" style={{ cursor: "pointer" }}>
        ...
        <div className="text-base text-canBlue flex items-center gap-2.5">
          ...
          <span className="text-base !text-canBlack font-medium">
            Canon:
            <span className="font-medium !text-canBlue ml-1">{item.namespace}</span>
          </span>
        </div>
      </li>
    );
  ...
  const TopicList = ({ data, searchValue, isAlgorithm }) => {
    ...
    return (
      <ul>
        {data.map((item) => (
          <TopicItem key={item?.topic_num || item?.link} item={item} searchValue={searchValue} isAlgorithm={isAlgorithm} />
        ))}
      </ul>
    );
  };
        ...
        <div className="flex lg:flex-row flex-col gap-10" id="elastic_topic_search_section_sidebar">
          ...
          <div className="pageContentWrap flex-1" id="elastic_topic_search_section_topic_heading">
            <div className={`bg-canGray lg:py-5 lg:px-8 py-4 px-4 rounded-xl mb-5`} >
              <div className="d-flex mb-2 align-items-center flex-wrap relative">
                <h4 data-testid="topic_heading" className="!mb-6 !text-base !font-semibold !text-canBlack" id="elastic_topic_search_section_topic_text">
                  Topic(S)
                </h4>
              </div>
              {loading ? (
          ...
          <div className={styles.search_lists}>
            {searchDataAll?.topic?.length ? (
              <div>
                {isAlgorithmSearch ? (
                  <TopicList data={displayedDataforAlgo} searchValue={searchValue} isAlgorithm={true} />
                ) : (
                  <TopicList data={displayedData} searchValue={searchValue} isAlgorithm={false} />
                )}
              </div>
            ) : (
