...
const handleVideoSelection = (id:number) => {
    ...
    return (
      ...
      <ul>
      {
       Object.values(videos)?.map(video=>(
          <li className={video.id === selectedVideoId && styles.active} 
          onClick={()=>handleVideoSelection(video.id)}>{video.label}</li>
        ))
      }
      </ul>
      <div>
      ...
      <Radio.Group className={styles.radioGroup} onChange={onChange} value={videoResolution}>
  ...
  // }}}
        ...
        config={{
          file:{
            tracks: [
               { kind: 'chapters', src: '/subs/introduction.vtt', srcLang: 'en', default: true, label:'Locations'}
            ]
          }
        }}
